                                ...
                                {lg.logicBlocks.map((lb, bIdx) => (
                                  <div key={lb.id} className="relative mb-4 last:mb-0 group/lbwrapper z-10">
                                     ...
                                     <div className="w-full flex items-center gap-2 p-3 bg-white rounded-xl border border-gray-200 hover:border-orange-300 transition-colors group/lb">
                                        ...
                                        <input placeholder="变量如(5%)" value={lb.preLeftTerm || ''} onChange={e => { const lgs = JSON.parse(JSON.stringify(cc.logicGroups)); lgs[lgIndex].logicBlocks[bIdx] = { ...lgs[lgIndex].logicBlocks[bIdx], preLeftTerm: e.target.value }; updateConfigurableCheckpoint(index, { logicGroups: lgs }); }} className="w-24 bg-gray-50 border border-transparent hover:border-gray-300 focus:border-orange-400 focus:bg-white rounded-lg px-2 py-1.5 text-xs outline-none transition-all" />
                                        <select value={lb.preOperator || ''} onChange={e => { const lgs = JSON.parse(JSON.stringify(cc.logicGroups)); lgs[lgIndex].logicBlocks[bIdx] = { ...lgs[lgIndex].logicBlocks[bIdx], preOperator: e.target.value }; updateConfigurableCheckpoint(index, { logicGroups: lgs }); }} className="w-16 bg-gray-50 border border-transparent hover:border-gray-300 focus:border-orange-400 focus:bg-white rounded-lg px-1 py-1.5 text-xs outline-none font-mono font-bold text-center transition-all appearance-none">
